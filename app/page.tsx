...
// FIX: Initialize state to null on first render to avoid server/client mismatch.
// The real time value is set in useEffect, which only runs on the client.
const CountdownTimer = ({ expiresAt }: { expiresAt: string }) => {
  const [timeInfo, setTimeInfo] = useState<{ text: string; seconds: number; percent: number } | null>(null)

  useEffect(() => {
    // Set immediately on mount so there's no flicker
